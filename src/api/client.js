...
class ApiClient {
	...
	async getLists(requestId = "lists") {
		return this.request((signal) => this.api.getLists({ signal }), requestId);
	}

	async getTags(requestId = "tags") {
		return this.request((signal) => this.api.getTags({ signal }), requestId);
	}
	...
	async updateList(id, data) {
		return this.request((signal) =>
			this.api.updateList(id, { ...data, signal }),
		);
	}

	async updateTag(id, data) {
		return this.request((signal) =>
			this.api.updateTag(id, { ...data, signal }),
		);
	}

	async updateBookmark(id, data) {
		return this.request((signal) =>
			this.api.updateBookmark(id, { ...data, signal }),
		);
	}

	async deleteBookmark(id) {
		return this.request((signal) => this.api.deleteBookmark(id, { signal }));
	}

	async exportBookmarks() {
		return this.request((signal) => this.api.exportBookmarks({ signal }));
	}

	async uploadAsset(file) {
		return this.request((signal) => this.api.uploadAsset(file, { signal }));
	}

	async createHighlight(data) {
		return this.request((signal) =>
			this.api.createHighlight({ ...data, signal }),
		);
	}

	async updateHighlight(id, data) {
		return this.request((signal) =>
			this.api.updateHighlight(id, { ...data, signal }),
		);
	}

	async deleteHighlight(id) {
		return this.request((signal) => this.api.deleteHighlight(id, { signal }));
	}

	async getBookmarkHighlights(bookmarkId) {
		return this.request((signal) =>
			this.api.getBookmarkHighlights(bookmarkId, { signal }),
		);
	}

	async checkHealth() {
		return this.request((signal) => this.api.checkHealth({ signal }));
	}
}
